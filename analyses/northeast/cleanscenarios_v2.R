#### Clean up scenarios using FIA and FVS data
### Randomly select 20 FIA plots from 20 years ago for each region and forest type
# * Only 10 FIA plots for Oak / hickory
...
setwd("~/Documents/git/dynamic_carbonaccounting/analyses/northeast/")
...
states <- c("MA", "ME", "NH", "NY", "VT") 
region <- "northeast"  

ecosubstokeep <- c("211", "221", "222", "M211")
...
dat_grouped <- treeplot %>%
  ...
  group_by(plt_cn) %>%
  summarize(across(c(ecosub, ecosubcd, statecd, countycd, plot, unitcd, lat, lon, fortypcd, owngrpcd, stdorgcd, 
                     invyr, remper, measdate, prev.plt.cn, prev.prev.plt.cn, prev.prev.prev.plt.cn, dstrbcd1, 
                     dstrbcd2, dstrbcd3, stdage, siteclcd, spgrpcd, kindcd, designcd, qa_status, TOTAL, siteclcd,
                     gsstk, elev,  slope, aspect, trtcd1, harvest_type1_srs, physclcd, rddistcd, baac, tpa.custom), function(x) first(na.omit(x))),
...
datprev = dat_grouped %>%
  ...
  select(plt_cn, invyr, measdate, dstrbcd1, dstrbcd2, dstrbcd3, harvest_type1_srs,  baac.perc.cut, hwp,
         trtcd1, total_mtco2eac, totalc, rd.ac.over, rd.ac.regen, volbfgrs.ac, baac, tpa.custom) %>%
  rename_with(~paste0(., ".prev")) %>%
...
datprevprev = datprev %>%
  ...
  select(plt_cn, invyr, measdate, dstrbcd1, dstrbcd2, dstrbcd3, harvest_type1_srs, total_mtco2eac,  baac.perc.cut, hwp,
         trtcd1, totalc, rd.ac.over, rd.ac.regen, volbfgrs.ac, baac, tpa.custom) %>%
...
colstodupe <- c("forestname", "forestsubtype", "stdage", "gsstk", "rd.ac.over", "rd.ac.regen",
                "invyr", "total_mtco2eac", "totalc", "volbfgrs.ac", "baac", "tpa.custom")
...
datclean$qmd.prev.prev.prev = sqrt(datclean$baac.prev.prev.prev/(0.005454 * datclean$tpa.custom.prev.prev.prev))
...
getgofc.oaks <- oaks %>%
  ...
  select(plt_cn, totalc, totalc.prev, totalc.prev.prev, totalc.prev.prev.prev,
         elev, stdage.prev.prev.prev, rddistcd, qmd.prev.prev.prev,
         unitname, baac.remv, harvest, nummills, rd.ac.over.prev.prev.prev,
         rd.ac.regen.prev.prev.prev, siteclcd, ecosub, forestname, forestsubtype, statecd) %>%
  na.omit() %>%
...
randomsubset.gof <- sample(unique(getgof.oaks$plt_cn), 8)


getgof.oaks.final <- getgof.oaks %>%
  filter(plt_cn %in% randomsubset.gof)

write.csv(getgof.oaks.final, "output/oak_gof_plots_v2.csv", row.names = FALSE)
...
donorpoolt.oaks <- oaks %>%
  select(plt_cn, invyr, invyr.prev, invyr.prev.prev, invyr.prev.prev.prev, 
         elev, stdage.prev.prev.prev, rddistcd, qmd.prev.prev.prev,
         unitname, baac.remv, harvest, nummills, rd.ac.over.prev.prev.prev,
         rd.ac.regen.prev.prev.prev, siteclcd, ecosub, forestname, statecd) %>%
  na.omit() %>%
  pivot_longer(cols = c(invyr:invyr.prev.prev.prev), names_to = "time", 
               values_to = "year") %>%
  mutate(time = ifelse(time == "invyr", 15, 
...
write.csv(allplots, "output/dynamic_matchedplots_oak_v2.csv", row.names = FALSE)
...
goodnessoffit <- matches %>%
  ...
  left_join(donorpool.oaks.final %>%
              select(plt_cn, elev, stdage.prev.prev.prev, rd.ac.over.prev.prev.prev, rddistcd,
                     qmd.prev.prev.prev,
                     siteclcd, rd.ac.regen.prev.prev.prev, 
                     ecosub), by= c("matches" = "plt_cn")) %>%
  distinct() %>%
  left_join(getgof.oaks.final %>%
              select(plt_cn, elev, stdage.prev.prev.prev, rd.ac.over.prev.prev.prev,
                     rddistcd, qmd.prev.prev.prev,
              ...
              rename(elev.proj=elev, stdage.proj=stdage.prev.prev.prev, 
                     ...
                     qmd.proj=qmd.prev.prev.prev, rddistcd.proj=rddistcd
                     ) %>%
...
SMDs = goodnessoffit %>%
  ...
  summarize(sdm_elev = abs(mean(elev.proj) - mean(as.numeric(elev))) / sd(elev.proj),
            ...
            sdm_rddistcd = abs(mean(rddistcd.proj) - mean(as.numeric(rddistcd))) / sd(rddistcd.proj),
  ...
  mutate(evaluation = ifelse(value >= 0.30, "High", "Okay"))
...
#<chr>         <dbl> <chr>     
#  1 sdm_elev     0.215  Okay      
#2 sdm_stdage   0.221  Okay      
#3 sdm_rddistcd 0.0510 Okay      
#4 sdm_rdover   0.142  Okay      
#5 sdm_siteclcd 0.384  High      
#6 sdm_rcregen  0.0665 Okay      
#7 sdm_qmd      0.241  Okay  
...
write.csv(addit_oaks, "output/northeast_oaks_scenarios_v2.csv", row.names=FALSE)
write.csv(project.bau, "output/northeast_oaks_projectvbau_fia_v2.csv", row.names = FALSE)



png("figures/dynamicvsstatic_oaks_extendedrotation_v2.png", 
    width=7,
...
ggplot(addit_oaks %>%
  ...
  geom_line() +
  ggtitle("c) Northeast: Oak / Hickory group") +
...
mbbs <- mbbies %>%
  filter(!is.na(prev.plt.cn), ecosub %in% c("M211", "211")) %>%
  mutate(get_removals = plyr::round_any(round(baac.remv, digits=2)*100, 5, f = floor),
         unitname = as.numeric(paste0(statecd, unitcd)))
...
write.csv(getgof.mbbs.final, "output/mbb_gof_plots_v2.csv", row.names = FALSE)
...
write.csv(allplots, "output/dynamic_matchedplots_mbb_v2.csv", row.names = FALSE)
...
### STDAGE is high at 0.28, otherwise looks good
...
mean(addit_mbbs$meangrow[addit_mbbs$method=="dynamic"]) ### 1.36
mean(addit_mbbs$meangrow[addit_mbbs$method=="static"]) ### 3.27

mean(addit_oaks$meangrow[addit_oaks$method=="dynamic"]) ### 1.55
mean(addit_oaks$meangrow[addit_oaks$method=="static"]) ### 2.8

write.csv(addit_mbbs, "output/northeast_mbbs_scenarios_v2.csv", row.names=FALSE)
write.csv(project.bau, "output/northeast_mbbs_projectvbau_fia_v2.csv", row.names = FALSE)

png("figures/dynamicvsstatic_mbbs_extendedrotation_v2.png", 
    width=7,
    height=6, units="in", res = 350 )
ggplot(addit_mbbs %>%
         mutate(meangrow = ifelse(method=="static" & time >0, mean(meangrow, na.rm=TRUE), meangrow)),
  ...
  #geom_ribbon(aes(ymin=meangrow-segrow, ymax=meangrow+segrow), alpha=0.3) +
  geom_line() +
  ggtitle("d) Northeast: Maple / beech / birch group") +
  theme_bw() + scale_color_d3(palette="category20c", name="Method") + scale_fill_d3(palette="category20c", name="Method") +
  xlab("Years since project start") + ylab("Additional MtCO2e/ac/yr")
dev.off()
