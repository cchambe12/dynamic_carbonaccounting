...
addit_oaks <- read.csv("output/southernapps_oaks_scenarios.csv")
...
m.dists <- MatchIt::matchit(tx ~  elev + stdage.prev.prev.prev + rd.ac.over.prev.prev.prev + siteclcd + 
                              rd.ac.regen.prev.prev.prev + ecosubcd, data = allplots,
                            method="nearest", 
                            distance="mahalanobis", replace=FALSE, ratio=5)
...
## Make longer
matches <- matches %>%
  pivot_longer(cols = c(V1:V5), names_to = "project", values_to = "matches") %>%
  mutate(project = rep(allplots$plt_cn[allplots$tx==1], each=5))
...
addit_mbbs <- read.csv("output/southernapps_mbbs_scenarios.csv")
