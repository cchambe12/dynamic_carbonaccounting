...
dat_initial <- dat_initial %>%
  mutate(volbfgrs.ac.initial = volbfgrs.ac.prev.prev.prev) %>%
  filter(!is.na(volbfgrs.ac.initial))
...
datclean <- dat_initial %>%
  ...
  filter(forestname %in% c(forestname1, forestname2),
         ...
         siteclcd <= 6,
         stdorgcd == 0,
         (invyr - invyr.prev) <= mean(invyr - invyr.prev, na.rm=TRUE) + 5
         )
...
oaks <- oakies %>%
  filter(!is.na(prev.plt.cn) & forestname == "Oak / hickory group", ecosub %in% c("M221", "221")) %>%
...
randomsubset.gof <- sample(unique(getgof.oaks$plt_cn), 20)
...
write.csv(addit_oaks, "output/centralapps_oaks_scenarios_v2.csv", row.names=FALSE)
write.csv(project.bau, "output/centralapps_oaks_projectvbau_fia_v2.csv", row.names = FALSE)
...
mbbs <- mbbies %>%
  filter(!is.na(prev.plt.cn), ecosub %in% c("211", "221")) %>%
  mutate(get_removals = plyr::round_any(round(baac.remv, digits=2)*100, 5, f = floor),
         unitname = as.numeric(paste0(statecd, unitcd)))
...
#### Grow-only plots
getgofc.mbbs <- mbbs %>%
  filter(get_removals == 0, numharvs==0, ecosub %in% c("211", "221")) %>%
  select(plt_cn, totalc, totalc.prev, totalc.prev.prev, totalc.prev.prev.prev,
         elev, stdage.prev.prev.prev, rddistcd, qmd.prev.prev.prev,
         unitname, baac.remv, harvest, nummills, rd.ac.over.prev.prev.prev,
         rd.ac.regen.prev.prev.prev, siteclcd, ecosub, forestname, statecd)%>%
...
write.csv(addit_mbbs, "output/centralapps_mbbs_scenarios_v2.csv", row.names=FALSE)
write.csv(project.bau, "output/centralapps_mbbs_projectvbau_fia_v2.csv", row.names = FALSE)
