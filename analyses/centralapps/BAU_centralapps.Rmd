...
if(file.exists(paste0("../../fiadata/", region, "/",
  ...
  for(i in c(states)){
getFIA(states = i, tables = c("TREE", "PLOT", "COND", "PLOTGEOM"), load=FALSE,
              dir=paste0("../../fiadata/", region))
  }
}

fia <- readFIA(paste0("../../fiadata/", region), common = FALSE, tables = c("PLOT", "TREE", "COND", "PLOTGEOM"))
...
tree = tree %>%
  ...
  dplyr::select(cn, dia, statuscd, baac, rd.ac, tpa.custom, volbfgrs.ac) %>%
...
dat_grouped <- treeplot %>%
  ...
  summarize(across(c(ecosub, ecosubcd, statecd, countycd, plot, unitcd, lat, lon, fortypcd, owngrpcd, stdorgcd, invyr, remper, measdate, prev.plt.cn, prev.prev.plt.cn, prev.prev.prev.plt.cn, dstrbcd1, 
                     ...
                     gsstk, elev,  slope, aspect, trtcd1, harvest_type1_srs, physclcd, rddistcd), function(x) first(na.omit(x))),
...
datprev = dat_grouped %>%
  ...
  dplyr::select(plt_cn, invyr, measdate, dstrbcd1, dstrbcd2, dstrbcd3, harvest_type1_srs,  baac.perc.cut, 
...
datprevprev = datprev %>%
  ...
  dplyr::select(plt_cn, invyr, measdate, dstrbcd1, dstrbcd2, dstrbcd3, harvest_type1_srs, total_mtco2eac,  baac.perc.cut, qmd, trtcd1, baac, shannon_index) %>%
...
dat = datprevprev %>%
  ...
  dplyr::select(plt_cn, qmd, trtcd1, baac, total_mtco2eac, shannon_index) %>%
  ...
  mutate(baac.remv = ifelse(harvest==1, max(baac.perc.cut, baac.perc.cut.prev, baac.perc.cut.prev.prev), 0),
         baac.perc.cut = ifelse(is.na(baac.perc.cut), 0, baac.perc.cut),
         baac.perc.cut.prev = ifelse(is.na(baac.perc.cut.prev), 0, baac.perc.cut.prev),
         baac.perc.cut.prev.prev = ifelse(is.na(baac.perc.cut.prev.prev), 0, baac.perc.cut.prev.prev),
...
prevdf <- prevdf %>%
  dplyr::select(plt_cn, colstodupe) %>%
  rename_with(~paste0(., ".prev")) %>%
  rename(prev.plt.cn = plt_cn.prev) %>%
  left_join(dat)
...
getmills <- left_join(fips, dat_initial %>% 
                        dplyr::select(statecd, countycd, unitcd) %>% distinct()) %>%
...
multharv <- datclean %>% 
  ...
  rename(`Ecoprovince` = ecosub) %>%
    ungroup() %>%
  dplyr::select(`Forest Type`, `Ecoprovince`, `Mult harvests`) 
...
harvtable <- datclean %>% 
  ...
  rename(`Ecoprovince` = ecosub) %>%
    ungroup() %>%
  dplyr::select(`Forest Type`,`Ecoprovince`, `# harvested`) 
...
grid.arrange(datclean %>% group_by(forestname, ecosub) %>%  
  ...
  rename(`Forest Type` = forestname) %>%
    rename(`Ecoprovince` = ecosub) %>%
    ungroup() %>%
  dplyr::select(`Forest Type`,`Ecoprovince`,  `# FIA plots`) %>% 
    left_join(harvtable) %>%
    left_join(multharv) %>%
 ...
 ## QMD change by removal amount - as per Catherine Henry's methods
  ...
  summarise(num_plot = sum(harvest == 1, na.rm=TRUE),
      ...
      diaharv <- speciestable %>%
        ...
        dplyr::mutate(harvested = statuscd.prev == 1 & statuscd == 2,
